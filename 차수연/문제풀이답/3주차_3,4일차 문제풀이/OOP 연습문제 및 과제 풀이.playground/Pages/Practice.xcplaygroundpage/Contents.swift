...
class Quadrangle { //1
  ...
  func dimension() -> (Int, Int) { //4
    return (width, height)
  }
  
  func setDimension(width: Int, height: Int) { //3
    self.width = width
    self.height = height
  }
}

let quadrangle = Quadrangle(width: 10, height: 5) //6
quadrangle.area()
...
class TV {
  ...
  func currentChannel() -> Int { //2
    guard isPowerOn else { print("전원이 꺼져있습니다."); return -1 }
    return channel
  }
  
  func currentVolume() -> Int { //2
    guard isPowerOn else { print("전원이 꺼져있습니다."); return -1 }
    return volume
  }
